...
void td::TransferData::DeltaEigen(td::TransferData* transfer) {
	...
	transfer->deltaEigenCartesian.x = (int)((delta.x * sin(transfer->currAngle / 180 * PI) * (-1) - delta.y * cos(transfer->currAngle / 180 * PI)) * MMperPIX);
	transfer->deltaEigenCartesian.y = (int)((delta.x * cos(transfer->currAngle / 180 * PI) * (-1) + delta.y * sin(transfer->currAngle / 180 * PI)) * MMperPIX);
}
